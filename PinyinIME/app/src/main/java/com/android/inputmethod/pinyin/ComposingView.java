...
public class ComposingView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        float width;
        int height;
        height = mFmi.bottom - mFmi.top + Config.mPaddingTop + Config.mPaddingBottom;

        if (null == mDecInfo) {
            width = 0;
        } else {
            width = Config.mPaddingLeft + Config.mPaddingRight + LEFT_RIGHT_MARGIN * 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float x, y;
        x = Config.mPaddingLeft + LEFT_RIGHT_MARGIN;
        y = -mFmi.top + Config.mPaddingTop;

        mPaint.setColor(mStrColorHl);
        mHlDrawable.setBounds(Config.mPaddingLeft, Config.mPaddingTop, getWidth()
                - Config.mPaddingRight, getHeight() - Config.mPaddingBottom);
    ...
    private void drawCursor(Canvas canvas, float x) {
        mCursor.setBounds((int) x, Config.mPaddingTop, (int) x
                + mCursor.getIntrinsicWidth(), getHeight() - Config.mPaddingBottom);
        mCursor.draw(canvas);
    }
